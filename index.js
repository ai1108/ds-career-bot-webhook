...
function getReplyByIntentAndCareer(intentName, career) {
  ...
  const normalizedCareer = CAREERS.includes(c) ? c : c.includes("研究所") ? "研究所" : "";
...
/* =========================
   ...
   (不改動原 7 個職位內容, 只補救 intent 沒命中時也能回)
========================= */
function inferKeyFromText(text) {
  ...
  // 學習路線
  if (/(學習路線|學習|準備方向|怎麼準備|roadmap|讀書順序|先學什麼|後學什麼)/.test(t)) return "learning_roadmap";

  // 作品集/專題
  if (/(專題|作品集|project|題目|可以做什麼專案)/.test(t)) return "project_suggestions";

  // 工具
  if (/(工具|tool|要用什麼|軟體|平台|推薦工具)/.test(t)) return "tools_recommendation";
