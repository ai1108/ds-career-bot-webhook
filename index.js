...
const replies = {
  ...
  tools_recommendation: {
    ...
    AI工程師:
      ...
      "🧷 評估與測試流程, 讓你能說清楚品質與限制\n" +
      "✨ 你越能把 AI 變得可靠可控, 你就越像能上線的 AI 工程師～!!",
...
function pickCareerFromRequest(req) {
  ...
  const c1 = normalizeText(p.career_type);
  const c2 = normalizeText(p.careerType);
  const c3 = normalizeText(p.career);
  const c4 = normalizeText(p.job);
  const c5 = normalizeText(p.position);

  // 有些人會用 entity list, 可能變成陣列
