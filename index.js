...
/* =========================
   ✅ NEW: 強化字串比對
   你的截圖是 Default Fallback Intent 進 webhook 了
   但 career 抓不到 => 用更強的「去空白/去符號」比對保證抓到
========================= */
...
function findCareerInText(text) {
  const t = normalizeForMatch(text);
  if (!t) return "";

  for (const c of CAREERS) {
    const cc = normalizeForMatch(c);
    if (cc && t.includes(cc)) return c;
  }

  // 容錯: 使用者說「研究所深造」之類
  if (t.includes(normalizeForMatch("研究所"))) return "研究所";
  return "";
}
...
function getReplyByIntentAndCareer(intentName, career) {
  ...
  const normalizedCareer = normalizeCareer(c);
...
/* =========================
   Infer aspect from queryText
   ✅ 先判斷「工具/作品集」, 避免被「學習」誤判
========================= */
...
function normalizeCareer(career) {
  ...
  // ✅ 同樣用強化比對避免「有空白/符號」抓不到
  const byText = findCareerInText(c);
  if (byText) return byText;
...
app.post("/webhook", (req, res) => {
  ...
  // ✅ 攔截 career overview / 職涯總覽：只要抓到職位，就不要讓 overview 吃掉
  ...
  // 2) intent 沒命中但文字有面向 + 有職位 → 直接回對應內容
